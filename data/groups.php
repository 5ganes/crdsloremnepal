...
class Groups
{
	...
	function getByParentURLName($urlname, $orderby = 'weight', $order = 'ASC', $limit = 100)
	{
		global $conn;
		$id = $this->getByURLName($urlname); $id = $id['id'];
		$sql = "SElECT * FROM groups WHERE parentId = '$id' order by $orderby $order limit $limit";
		// echo $sql;
		$result = $conn->exec($sql);
		return $result;
	}
